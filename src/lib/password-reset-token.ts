...
/** Invite-link lifetime: 7 days (per design decision). */
...
/** Invite expiry as ISO-8601 UTC string, 7 days out. */
export function inviteTokenExpiryIso(): string {
  return new Date(Date.now() + INVITE_TOKEN_TTL_MS).toISOString();
}
