                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {group.debts.length} buổi · <span className="text-destructive font-medium">{formatVND(group.totalOwed)}</span>
                        </div>
                      </div>
                    </div>

                    {/* Always visible: each debt with status + action */}
                    <div className="space-y-1.5 border-t pt-2">
                      {group.debts.map((debt) => (
                        <div key={debt.id} className="flex items-center justify-between gap-2 py-1">
                          <div className="flex items-center gap-2 min-w-0">
                            <div className="flex items-center gap-1 text-xs text-muted-foreground flex-shrink-0">
                              <Calendar className="h-3 w-3" />
                              {formatSessionDate(debt.sessionDate)}
                            </div>
                            <span className="text-xs font-medium">{formatVND(debt.totalAmount)}</span>
                            {debt.memberConfirmed ? (
                              <Badge variant="secondary" className="text-[10px] px-1.5 py-0">{t("waitingAdmin")}</Badge>
                            ) : (
